...
namespace EcsLte
{
    ...
    internal class EntityQueryComponentAdapter
    {
        ...
        private abstract class ComponentAdapter : IComponentAdapter
        {
            ...
            protected ComponentAdapter(ComponentConfig config)
            {
                Config = config;
            }
        ...
        private class BlittableComponentAdapter: ComponentAdapter
        {
            ...
            public override unsafe T GetComponent<T>()
            {
                return Marshal.PtrToStructure<T>((IntPtr)ComponentPtr);
            }
        ...
        {
            private IIndexDictionary _sharedComponentIndexDic;

            public BlittableSharedComponentAdapter(ComponentConfig config, SharedComponentIndexDictionaries sharedIndexDics)
                : base(config)
            {
                _sharedComponentIndexDic = sharedIndexDics.GetSharedIndexDic(config);
            }
        ...
        private class ManageComponentAdapter : ComponentAdapter
        {
            ...
            public override unsafe T GetComponent<T>()
            {
                return ((ManagedComponentPool<T>)_managePool).GetComponent(_componentIndex);
            }
